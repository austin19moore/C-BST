...
void traverseBST(BSTNode* root, BSTOrder userOrder) {
	...
	switch (userOrder) {
		...
		case POSTORDER:
		traverseBST(root->left, POSTORDER);
 
		traverseBST(root->right, POSTORDER);
		...
		traverseBST(root->left, PREORDER);
 
		traverseBST(root->right, PREORDER);
		break;
